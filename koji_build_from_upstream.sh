...
build_command="koji -p openkoji call build src=git+https://src.fedoraproject.org/rpms/{package}.git#{sha} target=$build_target channel=physical"
...
#do
  ...
  # 获取 SHA-1 值
  sha=$(git ls-remote https://src.fedoraproject.org/rpms/$package.git | grep refs/heads/$branch | awk '{print $1}')
